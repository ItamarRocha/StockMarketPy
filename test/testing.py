...
def get_data_from_yahoo(sector,reload_b3 = False):
    if reload_b3:
        tickers,trsh = tickers_func(sector)
    else:
...
def get_true_values(sector):
    tickers ,names = tickers_func(sector)
    start = dt.datetime(2018,7,2)
    end = dt.datetime(2019,6,25)
    df = pd.DataFrame()
    ...
    predi = dataset.loc[dataset['180d'].isnull()== False]
    predi = predi.fillna(0)
...
#test_compiled_dataframe(sector = 'construção',save = True)
def test_compiled_dataframe(sector,save = False):
    sector = 'construção'
    tickers,web = tickers_func(sector)
    ...
    ibov = pd.DataFrame(pd.read_csv("^BVSP.csv",index_col = 0)['Close'])
    ...
    for ticker, name in zip(tickers,web):
        ...
        df['ibov'] = ibov
        df['ibov'] = df['ibov'].shift(1)
        
        for i in range(1,10):
            df['-{}d'.format(i+1)] = df['Close'].shift(i) 
        ...
        dataset_final = dataset_final.join(divida).join(liquidezNsolvencia).join(fluxo_de_caixa)
        dataset_final = dataset_final.join(investimentos).join(dividendos)
        dataset_final = dataset_final.drop('4T 2018')
        for time in dataset_final.index:
            result = ""
            result = time[3:7]
            if time[0:1] == '1':
...
def test_preprocessed_dataframe(sector):
    ...
    for column in dataset.columns[23:83]: #ajeita as string e converte pra float
    ...
    return train_test,prediction,dataset
...
#df = daily_compiled_dataframe('construção')
